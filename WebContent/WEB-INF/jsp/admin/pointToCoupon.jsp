	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$(".addButton").click(function(){
				...
				data.pointToCouponID=null;
				if(checkFileds(data)){
					$.ajax({
						data:data,
						type:'post',
						url:"<s:property value="@com.haozhuoji.mall.util.Constant@admin_p+'/pointToCoupon_add.html'" />",
			...
			$(".updatePointToCouponList").click(function(){
				...
				curr.attr({"pointCouponID":old.find("td:eq(0)").text()});
				curr.find("td:eq(0)").text(old.find("td:eq(0)").text());
				curr.find("td:eq(2) input").val(old.find("td:eq(2)").text());
				curr.find("td:eq(3) select").val(old.find("td:eq(3)").attr("typeID"));
				curr.find("td:eq(4) input").val(old.find("td:eq(4)").text());
				curr.find("td:eq(5) input").val(old.find("td:eq(5)").text());
			...
			$(".cancelUpdate").click(function(){
				...
				$("tr[pointToCouponID='"+id+"']").css({"display":"table-row"});
			});
			$(".deletePointToCouponList").click(function(){
				if(confirm("确认删除！")){
					$.ajax({
						data:{pointToCouponID:$(this).closest("tr").find("td:eq(0)").text().trim()},
						type:'post',
						url:"<s:property value="@com.haozhuoji.mall.util.Constant@admin_p+'/pointToCoupon_delete.html'" />",
						success:function(data){
							if(data!="success"){
								alert(data);
							}else{
			...
			$(".confimrUpdate").click(function(){
				...
				if(checkFileds(data)){
					$.ajax({
						data:data,
						type:'post',
						url:"<s:property value="@com.haozhuoji.mall.util.Constant@admin_p+'/pointToCoupon_update.action'" />",
						success:function(data){
							if(data!="success"){
								alert(data);
							}else{
			...
			getFileds=function(tr){
				var data = {
					pointToCouponID:tr.find("td:eq(0)").text().trim(),
					points:tr.find("td:eq(2)").find("input").val().trim(),
					typeID:tr.find("td:eq(3)").find("select").val().trim(),
					beginDateTime:tr.find("td:eq(4)").find("input").val().trim(),
					endDateTime:tr.find("td:eq(5)").find("input").val().trim(),
			...
			checkFileds=function(data){
				if(data.points==""){
					alert("积分不能为空！");
				}else if(data.beginDateTime==""){
					alert("开始时间不能为空！");
				}else if(data.endDateTime==""){
					alert("结束时间不能为空！");
				}else if(!data.points.match("^\\d+$")){
					alert("积分格式错误！");
		...
		<div class="trueBody">
			...
			<table>
				...
				<s:iterator value="#request.pointToCouponList">
					<tr pointToCouponID="<s:property value="pointToCouponID" />" couponType="<s:property value="couponType" />">
						<td><s:property value="pointToCouponID" /></td>
						<td>
